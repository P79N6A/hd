 ...
 */
/**
 * _draggable - jQuery EasyUI
 * 
 */
(function($){
//	var isDragging = false;
	function drag(e){
		var state = $.data(e.data.target, '_draggable');
...
//		} else {
//			proxy = $(e.data.target);
//			$.data(e.data.target, '_draggable').handle.css('cursor', opts.cursor);
//		}
		proxy.css({
			left:e.data.left,
			top:e.data.top
...
//		drag(e);
	...
	$.fn._draggable = function(options, param){
		if (typeof options == 'string'){
			return $.fn._draggable.methods[options](this, param);
		}
		
		return this.each(function(){
			var opts;
			var state = $.data(this, '_draggable');
			if (state) {
				state.handle.unbind('._draggable');
				opts = $.extend(state.options, options);
			} else {
				opts = $.extend({}, $.fn._draggable.defaults, $.fn._draggable.parseOptions(this), options || {});
			}
			var handle = opts.handle ? (typeof opts.handle=='string' ? $(opts.handle, this) : opts.handle) : $(this);
			
			$.data(this, '_draggable', {
				options: opts,
				handle: handle
			});
			...
			handle.unbind('._draggable').bind('mousemove._draggable', {target:this}, function(e){
//				if (isDragging) return;
				if ($.fn._draggable.isDragging){return}
				var opts = $.data(e.data.target, '_draggable').options;
				if (checkArea(e)){
				...
				$(document).bind('mousedown._draggable', e.data, doDown);
				$(document).bind('mousemove._draggable', e.data, doMove);
				$(document).bind('mouseup._draggable', e.data, doUp);
//				$('body').css('cursor', opts.cursor);
			});
	...
	$.fn._draggable.parseOptions = function(target){
		var t = $(target);
		return $.extend({}, 
