...
function runLoadingMask(flag){
    var loadingMask = window.parent.document.getElementById("loading_icon");
    if(flag){
        $(loadingMask).show();
    }else{
        $(loadingMask).fadeOut(60);
    }
};
var BaseJs = function() {
...
function show_confirm(datatips) {
    ...
    var datamessage = '您确定要进行'+ datatitle +'操作？';
    bootbox.dialog({
        message: datamessage,
        title: datatitle,
        buttons: {
...
function quoteimg(){
    ...
    $(".quoteSave").click(function(){
        var textval=$(".textareaC").val();
        var sp=textval.split("\n");
        if(textval==""){
            alert("地址不可为空！");
        }else{
            for(var i=0;i<=sp.length-1;i++){
                var str = sp[i];
                var patt1 =/[h][t][t][p][s]?\:\/\/([^\s]+(?=\.(jpg|png|jpeg|gif|swf|svg))\.\2)/ig;
                window.reg_result=patt1.test(str);
                if(sp[i]!=""){
                    if (reg_result){
                        $(".urlquote").append(
                            '<span class="urlquotespan sortr-user sortr-handle" draggable="false">'+
                            '<div class="hideblock">' +
                            '<span class="setindeximg1" title="设为索引图"><i class="fa fa-filter font-grey-salt"></i></span>'+
                            '<span class="delimg"><i class="fa fa-remove font-grey-salt"></i></span>'+
                            '<textarea class="areaclass1 editchange" name="areaclass" readonly placeholder="请输入简介..."></textarea>'+
                            '<span class="confirmClass1">确定</span>'+
                            '<span class="editClass1">编辑</span>'+
                            '<span class="addClass1">插入</span>'+
                            '</div>'+
                            '<img src="'+sp[i]+'" style="width: 100%;height: 100%" />'+
                            '</span>'
                        );
                    }
                }else{
            ...
            if (reg_result==false){
                alert("请输入正确的http(s)://图片地址！");
            }
        }
    });
    $(".urlquote").on("mouseover",".urlquotespan",function(){
        $(this).children(".hideblock").show();
    });
    $(".urlquote").on("mouseleave",".urlquotespan",function(){
        $(this).children(".hideblock").hide();
    });
    $(".urlquote").on("click",".delimg",function(){
        $(this).parent().parent().remove();
    });
    $(".urlquote").on("mouseover",".delimg,.setindeximg1,.addClass1",function(){
        $(this).children().removeClass("font-grey-salt").addClass("font-blue");
    });
    $(".urlquote").on("mouseleave",".delimg,.setindeximg1,.addClass1",function(){
        $(this).children().removeClass("font-blue").addClass("font-grey-salt");
    });
    $(".urlquote").on("mouseover",".editClass1,.confirmClass1",function(){
        $(this).addClass("colorstyle");
    });
    $(".urlquote").on("mouseleave",".editClass1,.confirmClass1",function(){
        $(this).removeClass("colorstyle");
    });
    $(".urlquote").on("click",".confirmClass1",function(){
        $(this).hide();
        $(this).next().show();
    ...
    $(".urlquote").on("click",".editClass1",function(){
        $(this).hide();
        $(this).prev().show();
        $(this).prev().prev().removeClass("editchange");
        $(this).prev().prev().attr("readonly",false);
    });
    $(".urlquote").on("click",".setindeximg1",function(){
        var srcimg=$(this).parent().next("img").attr("src");
