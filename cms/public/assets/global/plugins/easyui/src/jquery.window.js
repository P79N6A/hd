...
//			if (param.top != null) opts.top = param.top;
	...
	function setProperties(target){
		...
		state.window._draggable({
			handle: '>div.panel-header>div.panel-title',
			disabled: state.options._draggable == false,
		...
		state.window._resizable({
			disabled: state.options._resizable == false,
...
//					$(this).appendTo('body');
	...
	$.fn.window.parseOptions = function(target){
		return $.extend({}, $.fn.panel.parseOptions(target), $.parser.parseOptions(target, [
			{_draggable:'boolean',_resizable:'boolean',shadow:'boolean',modal:'boolean',inline:'boolean'}
		]));
	};
