 ...
 */
class XianghuTask extends Task {
...
//        $this->cunAreaAction();
    ...
    public function catAction() {
        ...
        try {
            ...
            foreach($top as $t) {
                ...
                if(in_array($father, [
                    '看萧山', '看周边', '看中国', '看世界', '萧然文娱', '萧然健康', '网络教育台', '萧山理财帮','新闻其他','网络教育台（新）','今日关注(新闻部)', '视界', '广播', '专题', '湘湖视听', '领导专题','萧山原创影视','部门综合','拍客','村集合'
                ])) {
    ...
    public function newsAction() {
        ...
        $ids = implode(",",$town);
        $video = $this->db2->fetchAll("SELECT n.*,i.path FROM `cms_news` n left join cms_news_image i on n.id=i.news_id where n.topic_column_id =1047 and n.status=1");
    ...
    protected function parseImage($v, $type) {
        if($type == 'video') {
            $im = $this->db2->fetchOne("SELECT * FROM `cms_video_image` WHERE `video_id` = {$v['id']}");
            if(!empty($im)) {
                return $im['host'].$im['path'];
            }
        }
        if($type == 'news') {
            $im = $this->db2->fetchOne("SELECT * FROM `cms_news_image` WHERE `news_id` = {$v['id']}");
            if(!empty($im)) {
                return $im['path'];
            }
        }
        if($type == 'album') {
            $im = $this->db2->fetchOne("SELECT * FROM `cms_image` where album_id = {$v['id']} ORDER BY axis desc");
            if(!empty($im)) {
                return $im['path'];
            }
        }
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '萧山广电新媒体')->first();
            if($data) {
