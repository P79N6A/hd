 ...
 */
class HighcourtTask extends Task {

    protected $db2;
    protected $dbcity;
    protected $channel_id;
    protected $domain = 'cloudweb.cztv.com';
    ...
    public function govermentdepartAction() {
        //$depts = array('浙江省高级人民法院', '绍兴市', '杭州市', '湖州市', '嘉兴市', '丽水市', '衢州市', '宁波市', '台州市', '温州市', '金华市');
        //$father_id =13;

        //$depts = array('上城区法院', '下城区法院');
        //$father_id =25;

        //$depts = array('嵊州法院', '越城法院');
        //$father_id =26;

        //$depts = array('杭州中院','江干法院','滨江法院','富阳法院','拱墅法院','开发区法院','上城法院','淳安法院','建德法院','临安法院','余杭法院','桐庐法院');
        //$father_id =27;

        //$depts = array('安吉法院','德清法院','吴兴法院','长兴法院','湖州中院','南浔法院');
        //$father_id =28;

        //$depts = array('秀洲法院','平湖法院','嘉善法院','桐乡法院','南湖法院','嘉兴中院');
        //$father_id =29;

        //$depts = array('龙泉法院','云和法院','松阳法院','莲都法院','庆元法院','青田法院','缙云法院','景宁法院','丽水中院');
        //$father_id =30;

        //$depts = array('衢江法院','常山法院','柯城法院','龙游法院','开化法院','衢州中院');
        //$father_id =31;

        //$depts = array('江东法院','慈溪法院','北仑法院','宁海法院','象山法院','余姚法院','宁波中院','海事法院','海曙法院','镇海法院','江北法院','奉化法院','鄞州法院');
        //$father_id = 32;

        //$depts = array('玉环法院','温岭法院','黄岩法院');
        //$father_id = 34;

        //$depts = array('温州中院');
        //$father_id = 35;

        //$depts = array('金东法院','婺城法院');
        //$father_id = 36;
        ...
        foreach($depts as $d) {
            ...
            $government->name=$d;
            $government->father_id=$father_id;
            $government->pinyin=Cutf8py::encode($d, 'all');
            $government->pinyin_short=Cutf8py::encode($d);
            $government->channel_id = $this->channel_id;
            if($father_id==13) {
    ...
    public function topiccatAction() {
        ...
        try {
            // 创建视频分类
            $videoId = $this->createCat(0, '庭审直播专题', 0, 'web');
            // 开始处理顶级分类
            $top = $tree->getChild(0);
            foreach($top as $t) {
                $father = $tree->getValue($t);
    ...
    public function catalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_album_column` where project_id=8 and status=1');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function topiccatalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=24 and status=1 and type=2');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function catvideoAction() {
        //$column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=14 and status=1 order by id asc');
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=14 and status=1 order by id asc');
        $ignore = [
    ...
    public function videoAction() {
        ...
        foreach($video as $v) {
            ...
            if($hasSync) {
                ...
                $publishx = CategoryData::query()->andCondition('data_id', $xyz->new_id)->andCondition('category_id', $cat[$v['video_column_id']])->first();
                if(!$publishx->id){
    ...
    protected function cpVideo($data) {
        ...
        $cat = $this->catOld2New('cms_video_column');
        $categoryname = Category::findById($cat[$data['video_column_id']]);
        $goverment = GovernmentDepartment::query()->Where("channel_id=11 and name = '".$categoryname->name."'")->first();
        if($goverment) {
            $this->publishDepartment($new_id, $goverment->id);
        }
    ...
    protected function publish($new_id, $category_id, $data) {
        ...
        $xxx = CategoryData::query()->Where("data_id=".$new_id." and category_id=".$category_id)->first();
    ...
    public function newsAction() {
        ...
        //$video = $this->dbcity->fetchAll("SELECT n.*,i.path FROM `cms_news` n left join cms_news_image i on n.id=i.news_id where n.topic_column_id in (".$xxxstrging.") and n.status=1");
    ...
    protected function cpNews($data,$area='') {
        if(isset($data['video_id']) ){
            $old_video_id = $data['video_id'];
            $hasSync = $this->checkSync('cms_video', $old_video_id, 'data');
            if($hasSync) {
                $r = Data::getById($hasSync->new_id, $this->channel_id);
                $data['court_code'] = $data['params6'];
                $data['presiding_judges'] = $data['params2'];
                $data['collegiate_panel'] = $data['params3'];
                $data['prosecutor'] = $data['params4'];
                $data['defendant'] = $data['params5'];
                $data['juridical_days'] = $data['created2'];


                $r->update(array('data_template_id'=>1), Data::safeUpdateFields());
                DataExt::setExtValue($r->id, DataTemplateParams::getParams(1), $data, $r->partition_by);
                $cat = $this->catOld2New('cms_news_column');

                $this->publish($r->id, $cat[$data['news_column_id']], $data);
    ...
    protected function cpNews2($data,$area='') {
        var_dump($data);exit;
        // 检查是否已经cp
        $old_id = $data['id'];
        $old_table = 'cms_news';
    ...
    private function publishDepartment($data_id, $department){
        $model = new GovernmentDepartmentData();
        $model->save([
            'data_id'=>$data_id,
            'government_department_id'=>$department,
        ]);
    }
    ...
    protected function createData($v, $type) {
        ...
        $data['title'] = $v['title'];
        $data['intro'] = $v['intro'];
        $data['thumb'] = str_ireplace("http://cloudvideo.cztv.com", "http://v1.cztvcloud.com", $this->parseImage($v, $type));
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '浙江省高院庭审直播')->first();
            if($data) {
                ...
                $this->channel_id = $model->saveGetId([
                    ...
                    'channel_url' => 'http://cloudweb.cztv.com',
    ...
    protected function getTopicTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=43 and status=1 and type=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
    ...
    protected function getNewsColumnTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=14 and status=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
