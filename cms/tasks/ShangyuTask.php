 ...
 */
class ShangyuTask extends Task {

    protected $db2;
    protected $dbcity;
    protected $channel_id;
    protected $domain = 'www.sybtv.com';
    ...
    protected function getNewsColumnTreeByFatherId($father_id=999999999999) {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=10 and father='.$father_id.' and status=1');

        return $column;
    }

    public function subcattree($father_id) {
        $column = $this->getNewsColumnTreeByFatherId($father_id);
        $old_table="cms_news_column";
        $old_id = $father_id;
        $model = new Sync();
        ...
        if($hasSync&&count($column)) {
        $yunfather =  $hasSync->new_id;
        foreach($column as $item) {
            $this->createCat($yunfather, $item['title'], $item['id'], 'web', [], 'cms_news_column');
            $this->subcattree($item['id']);
        }
        }
    ...
    public function news2Action($father=613) {
        $columns = array();
        array_push($columns, $father);
        $index =0;
        while($index<count($columns)) {
            $father_id = $columns[$index];
            $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=10 and father='.$father_id.' and status=1');
            if(count($column)) {
            foreach($column as $item) {
                array_push($columns, $item['id']);
            }
            }
            $index++;
        }
    ...
    public function topiccatAction() {
        ...
        try {
            // 创建视频分类
            $videoId = $this->createCat(0, '上虞专题', 0, 'web');
            // 开始处理顶级分类
            $top = $tree->getChild(0);
            foreach($top as $t) {
                $father = $tree->getValue($t);
    ...
    public function catalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_album_column` where project_id=10 and status=1');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function catvideoAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=10 and status=1 order by id asc');
        $ignore = [
    ...
    public function newsAction() {
        ...
        $xxxstrging  = $this->news2Action(613); 
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '上虞广播电视台')->first();
            if($data) {
    ...
    protected function getTopicTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=40 and status=1 and type=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
    ...
    protected function getNewsColumnTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=10 and status=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
