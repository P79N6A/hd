 ...
 */
class TongxiangTask extends Task {

    protected $db2;
    protected $dbcity;
    protected $channel_id;
    protected $domain = 'v.txnews.com.cn';
    ...
    public function catAction() {
        ...
        try {
            ...
            foreach($top as $t) {
                ...
                if(in_array($father, [
                    '桐乡视听网','今日焦点', '互联网', '视频资讯', '国内国际', '桐乡新闻', '桐乡本地', '领导专区', '电视直播', '广播直播', '主持人风采'
                ])) {
    ...
    public function topiccatAction() {
        ...
        try {
            // 创建视频分类
            $videoId = $this->createCat(0, '桐乡专题', 0, 'web');
            // 开始处理顶级分类
            $top = $tree->getChild(0);
            foreach($top as $t) {
                $father = $tree->getValue($t);
    ...
    public function catalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_album_column` where project_id=11 and status=1');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function topiccatalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=42 and status=1 and type=2');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function catvideoAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=11 and status=1 order by id asc');
        $ignore = [
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '桐乡广播电视台')->first();
            if($data) {
                ...
                $this->channel_id = $model->saveGetId([
                    ...
                    'channel_url' => 'http://v.txnews.com.cn',
    ...
    protected function getTopicTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=42 and status=1 and type=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
    ...
    protected function getNewsColumnTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=11 and status=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
