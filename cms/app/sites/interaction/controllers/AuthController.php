 ...
 * 爆料
 */
    public function baoliaoAction() {
        $msg = "";
        $user = Auth::user();
        if ($this->request->isPost()) {
            $data = $this->request->getPost();
            $data = $this->xss_filter($data);
            ...
            $data['channel_id'] = (isset($data['channel_id']))?$data['channel_id']:$user->channel_id;
            $data['user_id'] = (isset($data['user_id']))?$data['user_id']:$user->id;
            $data['username'] = (isset($data['username']))?$data['username']:$user->name;
    ...
    //发邮件
    function send_mail($toEmail,$fromEmail,$fromName,$subject,$content) {
        $url = 'http://api.sendcloud.net/apiv2/mail/send';
        $API_USER = 'happycncico';
        $API_KEY = '12QXssdxbjaIJcBT';
    ...
    public function feedbackAction(){
        ...
        if( Request::has("type") &&  Request::has("company") &&  Request::has("name") && Request::has("channelId") &&
            Request::has("phone")&&  Request::has("subject")&&  Request::has("content") && Request::has("tomail")){

            $fromEmail = "website@cncico.com";
            $fromName = "员工互动";
            ...
            $typeArr = ["投诉举报","建议意见","个人诉求","选人用人","其他"];
            $subject = "[".$typeArr[$type]."]". Request::get("subject");
            $content = Request::get("content") ."<p><br/>单位:$company"."<br/>姓名:$name"."<br/>联系电话:$phone";

            $mailConf =F::getConfig('baoliao_mail', Request::get("channelId"));
            if( Request::get("tomail ") == 0 ){
                $toEmail = $mailConf["chairman_mail"];//董事长
            }else{
                $toEmail = $mailConf["manager_mail"];//总经理
            }


            $res = $this->send_mail($toEmail,$fromEmail,$fromName,$subject,$content);
            $statusCode = json_decode($res,true)["statusCode"];
            $this->jsonp(["statusCode"=>$statusCode]);
        }else{
            $this->jsonp(["statusCode"=>404]);
        }
