 ...
 * Time: 10:35
 */
class UgcyunLiveRoomController extends \BackendBaseController {
    private $quaname = array();


    public function indexAction() {
        $channel_id = Session::get('user')->channel_id;
        $data = UgcyunLiveRoom::getAll($channel_id);
        $roominfo = [];
        if(!empty($data->models)) {
            ...
            foreach ($rooms as $room) {
                ...
                $status = UgcyunLive::getWorkStatus($anchor->id);
                $line_num = UgcyunLive::getLineNum($anchor->id);
    ...
    public function addAction() {
        ...
        $adminids = AdminExt::getAdminIdsByUgcGroup([$def_gid]);
        if($adminids) {
            $userList = Admin::getUnLockAdminKVList($adminids);
        }
        else {
            $userList = array();
        }
    ...
    public function detailAction() {
        ...
        if ($room = $model->findFirst($id)) {
            ...
            $anchor = Admin::findFirst($room->admin_id);
            $room_ext['item_no'] = 'zbt' . str_pad($room->id, 5, '0', STR_PAD_LEFT);
            $room_ext['anchor_name'] = $anchor->name;
            $room_ext['audience'] = 50;
            $room_ext['line'] = UgcyunLive::getLineNum($room->admin_id);
            $online_stream_list = UgcyunLive::getOnlineList($room->admin_id);
            $data = array();
