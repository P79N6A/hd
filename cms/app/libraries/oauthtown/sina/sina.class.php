...
class SaeTOAuthV2 {
    ...
    function http($url, $method, $postfields = NULL, $headers = array()) {
        ...
        if (CZTV_PROXY_ST == 1) {
            curl_setopt($ci, CURLOPT_PROXYAUTH, CURLAUTH_BASIC); //代理认证模式
            curl_setopt($ci, CURLOPT_PROXY, CZTV_PROXY_IP); //代理服务器地址
            curl_setopt($ci, CURLOPT_PROXYPORT, CZTV_PROXY_PORT); //代理服务器端口
            curl_setopt($ci, CURLOPT_PROXYTYPE, CURLPROXY_HTTP); //使用http代理模式
        }
