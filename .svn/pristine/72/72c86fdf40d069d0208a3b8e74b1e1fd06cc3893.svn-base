 ...
 */
class QuzhouTask extends Task {

    protected $db2;
    protected $dbcity;
    protected $channel_id;
    protected $domain = 'tv.qz123.com';
    ...
    public function topiccatAction() {
        ...
        try {
            // 创建视频分类
            $videoId = $this->createCat(0, '衢州专题', 0, 'web');
            // 开始处理顶级分类
            $top = $tree->getChild(0);
            foreach($top as $t) {
                $father = $tree->getValue($t);
    ...
    public function catalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_album_column` where project_id=4 and status=1');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function topiccatalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=30 and status=1 and type=2');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function catvideoAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=4 and status=1 order by id asc');
        $ignore = [
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '衢州广播电视台')->first();
            if($data) {
    ...
    protected function getTopicTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=30 and status=1 and type=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
    ...
    protected function getNewsColumnTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=4 and status=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
