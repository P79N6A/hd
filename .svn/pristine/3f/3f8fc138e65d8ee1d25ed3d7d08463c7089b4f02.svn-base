...
class PinYin {
    ...
    public static function to_first($s, $lower = true) {
        $ascii = ord($s{0});
        if ($ascii > 0xE0) {
            $s = self::utf8_to_gb2312($s{0} . $s{1} . $s{2});
        } elseif ($ascii < 0x80) {
    ...
    public static function to_first_str($words) {
        $result = "";
        for ($i = 0; $i < PinYin::utf8_strlen($words); $i++) {
            $str = mb_substr($words, $i, 1, "UTF-8");
            $result = $result . PinYin::to_first($str);
        }
        return $result;
    }

    public static function to_str($words) {
        $result = "";
        for ($i = 0; $i < PinYin::utf8_strlen($words); $i++) {
            $str = mb_substr($words, $i, 1, "UTF-8");
            $result = $result . PinYin::to($str);
        }
        return $result;
    }

    private static function utf8_strlen($string = null) {
