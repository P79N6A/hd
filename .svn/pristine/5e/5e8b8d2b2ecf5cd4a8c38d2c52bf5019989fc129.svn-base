...
class AuthAssign extends Model {
    ...
    static function getRoleId($user) {        
        $assignrole = AuthAssign::find(array('conditions' => "type = 1 AND channel_id= {$user->channel_id} AND user_id ={$user->id}"));
        $roleids = [];
    ...
    public static function getOne($assignid) {
        ...
        return AuthAssign::findFirst($parameters);
    }
    
    public static function getAssignRole($admin_id) {
        $parameters = array();
        $parameters['conditions'] = "channel_id=" . Session::get('user')->channel_id;
        $parameters['conditions'] .= " and user_id=" . $admin_id;
        $parameters['conditions'] .= " and type=1";
