...
var ZYFILE = {
	...
	funUploadFiles : function(){
		...
		// 遍历所有文件  ，在调用单个文件上传的方法
		$.each(this.uploadFile, function(k, v){
			self.funUploadFile(v,k);
		});
	},
	// 上传单个个文件
	funUploadFile : function(file,order){
		var self = this;  // 在each中this指向没个v  所以先将this保留
        order =order||0;
		...
		if($('#uploadList_'+ file.index).length>0) {
			formdata.append("intro", $('#uploadList_' + file.index).children().find(".areaclass").val());
		}
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '                   <div class="urlQuo" data-toggle="modal" data-target="#myModal-img">链接引用</div>';
				    ...
				    html += '						<div class="uploadImg1" style="width:115px">';
				    html += '							<img class="upload_image" src="../../assets/global/plugins/zyupload/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					html += '<div id="uploadList_'+ file.index +'" class="upload_append_list sortr-user sortr-handle" draggable="false"">';
					html += '	<div class="file_bar" style="height:100%;display:none">';
					html += '		<div style="padding:5px;">';
					html += '			<p class="file_name"  style="display:none" title="'+file.name+'">' + file.name + '</p>';
					html += '           <span class="setindeximg" title="设为索引图"><i class="fa fa-filter"></i></span>';
					html += 			editHtml;  // 编辑按钮的html
					html += 			delHtml;   // 删除按钮的html
					html += '           <textarea class="areaclass editchange" name="areaclass" placeholder="请输入简介..." readonly></textarea>';
					html += '           <span class="confirmClass" style="display:none">确定</span>';
					html += '           <span class="editClass">编辑</span>';
					html += '           ';
					html += '		</div>';
					html += '	</div>';
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox sortable ui-sortable">';
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px;max-width:'+imgWidth+'px;max-height:'+imgHeight+'px;">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" draggable="false"" src="' + e.target.result + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width);" />';                                                                 
					html += '		</div>';
					html += '	</a>';
...
//								}
						...
						var funBindClass = function(){
                            $(".editClass,.confirmClass").mouseover(function(e){
                                   $(this).addClass("colorstyle");
							});
							$(".editClass,.confirmClass").mouseleave(function(e){
                                   $(this).removeClass("colorstyle");
							});
							$(".confirmClass").click(function(){
								$(this).hide();
								$(this).next().show();
								$(this).prev().addClass("editchange");
								$(this).prev().attr("readonly",true);
							});
							$(".editClass").click(function(){
								$(this).hide();
								$(this).prev().show();
								$(this).prev().prev().removeClass("editchange");
								$(this).prev().prev().attr("readonly",false);
							});
							$(".setindeximg").click(function(){
								var srcimg=$(this).parent().parent().next(".imgBox").children().children().attr("src");
								$(".dropify-render img").attr("src",srcimg);
								$(".dropify-filename-inner").text(srcimg);
							});
							$(".upload_append_list").mouseover(function(e){
                                $(this).children(".file_bar").show();
							});
							$(".upload_append_list").mouseleave(function(e){
                                $(this).children(".file_bar").hide();
							});
						};
						//绑定拖拽事件
						var dragableScript = function(){
                            //上传图片部分拖拽
    new Sortable(preview, {
        ...
        onAdd: function (evt) {
            console.log('onAdd.preview:', evt.item);
        },
        onUpdate: function (evt) {
            console.log('onUpdate.preview:', evt.item);
        },
        onRemove: function (evt) {
            console.log('onRemove.preview:', evt.item);
        },
        onStart: function (evt) {
            console.log('onStart.preview:', evt.item);
        },
        onEnd: function (evt) {
            console.log('onEnd.preview:', evt.item);
        }
    });
						};
						funDealtPreviewHtml();		
