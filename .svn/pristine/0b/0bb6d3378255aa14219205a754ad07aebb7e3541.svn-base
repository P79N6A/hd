 ...
 */
class YuyaoTask extends Task {

    protected $db2;
    protected $dbcity;
    protected $channel_id;
    protected $domain = "www.yysee.net";
    ...
    public function topiccatAction() {
        ...
        try {
            // 创建视频分类
            //$videoId = $this->createCat(0, '专题', 0, 'web');
            // 开始处理顶级分类
            $top = $tree->getChild(0);
            foreach($top as $t) {
                $father = $tree->getValue($t);
                if(in_array($father, [
                    '看余姚', '看见吧', '看天下', '看生活 ', '看栏目', '听广播', '看广电', '市委书记视频专区', '市长视频专区', '广电精品'
                ])) {
    ...
    public function catalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_album_column` where project_id=1 and status=1');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function topiccatalbumAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=25 and status=1 and type=2');
        $ignore = [
            'none'
        ];
        // 创建栏目
    ...
    public function catvideoAction() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_video_column` where project_id=1 and status=1 order by id asc');
        $ignore = [
    ...
    protected function createChannel() {
        if($this->channel_id == null) {
            $data = Channel::query()->andCondition('name', '余姚广播电视台')->first();
            if($data) {
                ...
                $this->channel_id = $model->saveGetId([
                    ...
                    'channel_url' => 'http://www.yysee.net',
    ...
    protected function getTopicTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_topic_column` where topic_id=25 and status=1 and type=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
    ...
    protected function getNewsColumnTree() {
        $column = $this->dbcity->fetchAll('SELECT * FROM `cms_news_column` where project_id=1 and status=1');
        $tree = new Tree();
        if(!empty($column)) {
            foreach($column as $v) {
                $tree->setNode($v['id'], $v['father'], $v['title']);
